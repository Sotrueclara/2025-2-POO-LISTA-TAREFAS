...
public class Operacoes
{
         ...
         using (var conexao = new MySqlConnection(connectionString))
    {
        ...
        string sql = @"
            UPDATE tarefa
               SET nome         = @nome,
                   ...
                   status       = @status
             WHERE id = @id;"; // WHERE para não alterar tudo
        ...
        using (var cmd = new MySqlCommand(sql, conexao))
        {
            ...
            cmd.Parameters.AddWithValue("@descricao", tarefa.Descricao);
            cmd.Parameters.AddWithValue("@dataCriacao", tarefa.DataCriacao);
            cmd.Parameters.AddWithValue("@dataExecucao", (object?)tarefa.DataExecucao ?? DBNull.Value);
            cmd.Parameters.AddWithValue("@status", tarefa.Status);
         ...
         using (var conexao = new MySqlConnection(connectionString))
    {
        ...
        string sql = @"DELETE FROM tarefa WHERE id = @id;"; // WHERE evita deletar tudo
